...
| URL | 内容 |
|---|---|
| `keii-dev.com/` | トップページ（カテゴリ一覧） |
| `keii-dev.com/game/` | GAME — ゲーム攻略・ビルド情報 |
| `keii-dev.com/game/poe2/` | Path of Exile 2 のビルド情報・攻略メモ |
| `keii-dev.com/book/` | BOOK — 読書メモ・書評・要約 |
| `keii-dev.com/dev/` | DEV — 開発ログ・技術メモ |

全ページ共通のナビヘッダー（TOP / GAME / BOOK / DEV）でページ間を移動できる。
...
| セクション | ディレクトリ |
|---|---|
| BUILDS | `public/game/poe2/builds/` |
| GAMEPLAY NOTES | `public/game/poe2/notes/` |
| TIPS & MECHANICS | `public/game/poe2/tips/` |
...
- YAML フロントマター（`---` で囲まれたメタデータ）は自動で除去される
...
`public/game/poe2/content.json` の対応する配列にファイル名を追記：
...
1. `src/game/` に新しいディレクトリと `index.html` + `.ts` を作成（poe2 をコピーして修正）
2. `public/game/` にコンテンツ用ディレクトリを作成
3. `vite.config.ts` の `rollupOptions.input` にエントリを追加
4. `src/game/index.html` にリンクカードを追加
5. push してデプロイ
...
2. `.ts` で `createNav` を呼び出してナビヘッダーを表示
3. `src/components/nav.ts` の `navItems` にカテゴリを追加
4. `vite.config.ts` の `rollupOptions.input` にエントリを追加
5. `src/index.html` のトップページにカテゴリカードを追加
6. push してデプロイ
...
`src/components/nav.ts` で全ページ共通のナビゲーションを管理。
カテゴリの追加・変更は `navItems` 配列を編集するだけ。
